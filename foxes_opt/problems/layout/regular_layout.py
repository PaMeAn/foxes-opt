...
class RegularLayoutOptProblem_OLD(FarmVarsProblem):
    """
...
class RegularLayoutOptProblem(FarmVarsProblem):
    ...
    def initialize(self, verbosity=1, **kwargs):
        ...
        try:
            self._xy0 = np.array([*shapely.Polygon(b.points).centroid.coords[0]])
        except:
            print("Area is a union, will take lower left coordinate point")
            self._xy0 = pmin
        self._halfspan = (pmax - pmin) / 2
        self._halflen = np.linalg.norm(self._halfspan)
        self.max_spacing = 2 * (self._halflen + self.min_spacing)
    ...
    def max_values_float(self):
        ...
        return np.array(
            [
                self.max_spacing,
                self.max_spacing,
                1,  # defined relative to dx ! # self._halflen / 2,
                1,  # self._halflen / 2,
                90,
            ],
            dtype=config.dtype_double,
